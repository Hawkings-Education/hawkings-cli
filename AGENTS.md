id: "rewrite-your-cli-for-ai-agents"
title: "You Need to Rewrite Your CLI for AI Agents"
description: "Human DX optimizes for discoverability. Agent DX optimizes for
  predictability. What I learned building a CLI for agents first."
...
faq:
  - question: "Do I need to rewrite my CLI from scratch?"
    answer: "No. Most of these patterns can be added incrementally. Start with
      --output json and input validation, then layer on schema introspection and
      skill files."
  - question: "What if my CLI doesn't wrap a REST API?"
    answer: "The principles still apply. Any CLI that agents invoke needs
      machine-readable output, input hardening, and explicit documentation of
      invariants. The schema introspection pattern is most valuable for
      API-backed CLIs, but --describe or --help --json works for anything."
  - question: "How do I handle auth for agents?"
    answer: "Environment variables for tokens and credential file paths. Service
      accounts where possible. Avoid flows that require a browser redirect."
  - question: "Is MCP worth the investment?"
    answer: "If your CLI wraps a structured API, yes. MCP eliminates shell escaping,
      argument parsing ambiguity, and output parsing. The agent calls a typed
      function instead of constructing a string."
  - question: "How do I test that my CLI is agent-safe?"
    answer: "Fuzz your inputs with the kinds of mistakes agents make, such as path
      traversals, embedded query params, double-encoded strings, and control
      characters. --dry-run should catch issues before they hit your API."
canonicalURL: "https://justin.poehnelt.com/posts/rewrite-your-cli-for-ai-agents/"
relativeURL: "/posts/rewrite-your-cli-for-ai-agents/"
markdownURL: "/posts/rewrite-your-cli-for-ai-agents.md"
---
```bash
...
**Update:** I wrote a followup exploring what happens when you add protocol layers like MCP on top of these APIs: [The MCP Abstraction Tax](https://justin.poehnelt.com/posts/mcp-abstraction-tax).

The real question: what does it actually look like to build for this?

## Raw JSON Payloads > Bespoke Flags[](#raw-json-payloads--bespoke-flags)
...
## Schema Introspection Replaces Documentation[](#schema-introspection-replaces-documentation)
...
## Input Hardening Against Hallucinations[](#input-hardening-against-hallucinations)
...
## Ship Agent Skills, Not Just Commands[](#ship-agent-skills-not-just-commands)
...
## Multi-Surface: MCP, Extensions, Env Vars[](#multi-surface-mcp-extensions-env-vars)
...
## Safety Rails: Dry-Run + Response Sanitization[](#safety-rails-dry-run--response-sanitization)
...
[You Need to Rewrite Your CLI for AI Agents](https://justin.poehnelt.com/posts/rewrite-your-cli-for-ai-agents/) © 2026 by [Justin Poehnelt](https://justin.poehnelt.com/) is licensed under CC BY-SA 4.0
